...
const emailForgotPassword = async (call, callback) => {
    ...
    try {
        const response: EmailForgotPasswordResponse = await userEmailForgotPasswordControllerImpl.forgotPassword(request.userEmailForgotPasswordRequest, request.userDeviceInformation, context, labels);
        toRet = response;
