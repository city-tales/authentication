...
import {
    GRPCUserLoginType,
    UserLoginType,
} from "../database/types/user_login.js";
...
class UserLoginControllerImpl implements UserLoginController {
    mapUserLoginSchema(userInfo: GRPCUserLoginType): UserLoginType {
        const sanitisedUserLoginInfo: GRPCUserLoginType =
            Helper.convertToType<GRPCUserLoginType>(
    ...
    async loginUser(
        ...
        const userLoginSchemaInfo: UserLoginType =
            this.mapUserLoginSchema(userInfo);
        ...
        try {
            const isKeyInRedis: LoginResponse =
                await userLoginRepositoryImpl.checkUserInRedis(
                    userLoginSchemaInfo,
                    context,
                    labels,
                );
            if (
                Helper.isNeitherNullNorUndefinedNorEmpty(isKeyInRedis.token) &&
                isKeyInRedis.statusCode === Constants.STATUS_CODES.OK
            ) {
                response = isKeyInRedis;
            } else if (
                isKeyInRedis.message ===
                Constants.LOGIN_MESSAGE.PASSWORD_DO_NOT_MATCH
            ) {
                response = isKeyInRedis;
            } else {
                const userResponse: LoginResponse =
                    await userLoginRepositoryImpl.loginUser(
                        userLoginSchemaInfo,
                        deviceLoginSchemaInfo,
                        context,
                        labels,
