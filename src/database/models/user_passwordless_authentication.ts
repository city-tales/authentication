...
interface UserPasswordlessAuthentication {
    ...
    authenticateUserEmailForPasswordless(
        email: string,
        _id: string,
        username: string,
        deviceInfo: DeviceType,
...
class UserPasswordlessAuthenticationImpl
    implements UserPasswordlessAuthentication
{
    async checkIfUserExists(
        ...
        try {
            ...
            if (
                ...
                await queueEmployee.addJobToQueue(
                    ...
                    Constants.DB.SAVE_IN_REDIS,
                    {
                        ...
                        timeout:
                            Constants.DB_TIMEOUTS.SHORT_CACHE_DB_REDIS_TIMEOUT,
                    },
                );
                ...
                response.token =
                    Helper.generatePasswordlessAuthenticationAuthToken(
                        prepareExistingUserToken,
                        deviceInfo,
                        labels.operation,
                    );
            } else {
                response._id =
                    Constants.PASSWORDLESS_AUTHENTICATION_MESSAGE.EMPTY;
                response.message =
                    Constants.PASSWORDLESS_AUTHENTICATION_MESSAGE.NO_CONTENT;
            }
            ...
            response.message = Helper.isNeitherNullNorUndefinedNorEmpty(
                ...
                ? error.message
                : Constants.DB_ERRORS.INSERTION_FAILED;
    ...
    async authenticateUserEmailForPasswordless(
        email: string,
        _id: string,
        username: string,
        deviceInfo: DeviceType,
        ...
        try {
            ...
            if (
                ...
                if (!data.is_email_verified || !data.is_passwordless) {
                    ...
                    const updatedQueryResponse =
                        await Helper.executeQueryAsyncWithoutLock(
                            context,
                            updateQuery,
                            valuesArray,
                            Constants.DB_ERRORS.UPDATE_FAILED,
                            labels,
                        );

                    if (
                        Helper.isUpdateQuerySuccessful(
                            updatedQueryResponse.command,
                            updatedQueryResponse.rowCount,
                        )
                    ) {
    ...
    async createUser(
        ...
        try {
            const queryResponse =
                await Helper.executeMultipleQueryAsyncWithoutLock(
                    context,
                    usersAuthDataQuery,
                    Constants.DB_ERRORS.INSERTION_FAILED,
                    labels,
            ...
            if (queryResponse.length) {
                response.success = true;
                response.message =
                    Constants.PASSWORDLESS_AUTHENTICATION_MESSAGE.NEW_USER;
                ...
                await queueEmployee.addJobToQueue(
                    ...
                    Constants.DB.SAVE_IN_REDIS,
                    {
                        ...
                        value: Helper.serialiseRedisKeyValues(redisEmailValue),
                        timeout:
                            Constants.DB_TIMEOUTS
                                .VERY_SHORT_CACHE_DB_REDIS_TIMEOUT,
                    },
                );
            }
        } catch (error) {
...
export const userPasswordlessAuthenticationImpl =
    new UserPasswordlessAuthenticationImpl();
